    ...
    public class Main extends JFrame implements KeyListener, MouseInputListener {
       ...
       // static final int GRASS = 0;//GRASS
        ...
        static {//заполняем сетку случайными часламми  от 0 до 3
    ...
    //  for (int i = 0; i < COUNT_ZOMBIE; i++) {}
                ...
                for (int i = 0; i < cellsX; i++) {
                    for (int j = 0; j < cellsY; j++) {
                        ...
                        switch (grid[i][j]) {
                            case GRASS:
                                g.setColor(new Color(86, 229, 26));
                                break;
                            case YELLOW_GRASS :
                                g.setColor(new Color(186, 173, 57));
                                break;
                            case WATER:
                                g.setColor(new Color(4, 57, 193));
                                break;
                            case SWAMP:
                                g.setColor(new Color(117, 154, 48));
                                break;
                            default:
                        ...
                        g.drawString(grid_find_way[i][j]+"" ,gridX + cellSizeX * i, gridY + cellSizeY * j+ cellSizeY * 3 / 4);
                         */
                    }
                }
        ...
        public static void recalculatePaths(int tx, int ty) {/////////лишние методы...
            for (int i = 0; i < grid_find_way.length; i++) {
                for (int j = 0; j < grid_find_way[0].length; j++) {
                    if (grid_find_way[i][j] >= 0) grid_find_way[i][j] = 0;
                }
            }
            grid_find_way[tx][ty] = 1;
            int cur = 1;
            ...
            do {
                ...
                for (int i = 0; i < grid_find_way.length; i++) {
                    for (int j = 0; j < grid_find_way[0].length; j++) {
                        if (grid_find_way[i][j] != 0) continue;
                        if ((i - 1 >= 0 && grid_find_way[i - 1][j] == cur) ||
                                (j - 1 >= 0 && grid_find_way[i][j - 1] == cur) ||
                                (i + 1 < cellsX && grid_find_way[i + 1][j] == cur) ||
                                (j + 1 < cellsY && grid_find_way[i][j + 1] == cur)) {
                        if( grid[i][j]!=WATER) {
                            grid_find_way[i][j] = cur + 1;
                        } else {
                            grid_find_way[i][j]=100;
                        }
                            overwritten = true;
        ...
        public static void moveBot(int zombieIndex) {/////////лишние методы...
                ...
                if ((zombieX[zombieIndex] - 1 >= 0 && grid_find_way[zombieX[zombieIndex] - 1][zombieY[zombieIndex]] == grid_find_way[zombieX[zombieIndex]][zombieY[zombieIndex]] - 1)) {
                    zombieX[zombieIndex]--;
                } else if (zombieY[zombieIndex] - 1 >= 0 && grid_find_way[zombieX[zombieIndex]][zombieY[zombieIndex] - 1] == grid_find_way[zombieX[zombieIndex]][zombieY[zombieIndex]] - 1) {
                    zombieY[zombieIndex]--;
                } else if ((zombieX[zombieIndex] + 1 < cellsX && grid_find_way[zombieX[zombieIndex] + 1][zombieY[zombieIndex]] == grid_find_way[zombieX[zombieIndex]][zombieY[zombieIndex]] - 1)) {
                    zombieX[zombieIndex]++;
                } else if (zombieY[zombieIndex] + 1 < cellsY && grid_find_way[zombieX[zombieIndex]][zombieY[zombieIndex] + 1] == grid_find_way[zombieX[zombieIndex]][zombieY[zombieIndex]] - 1) {
                    zombieY[zombieIndex]++;
                }
        ...
        public void turn(int dx, int dy) {
            if (grid[playerX + dx][playerY + dy] != WATER) {//Запрещаем ходить в воду и в лаву
                playerX += dx;
                playerY += dy;
            }
        }
